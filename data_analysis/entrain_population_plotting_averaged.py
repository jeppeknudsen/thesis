...
data_idx = 5 # Choose which file to analyse
run_idx = 3
...
data_corrected_averaged_arr = []
for data_run in data_corrected_arr:
    x_av, y_av = get_ave_values(x_nutlin, data_run, n=n_average)
    data_corrected_averaged_arr.append(y_av)
...
for n_data_run in range(data_num):
    ...
    peak_height = None
    peak_threshold = None
    peak_distance = 5 # 15 er 2.5 h
    peak_prominence_factor = 1/8
    if np.max(data_corrected_averaged_arr[n_data_run])>2000:
        peak_prominence = 2000*peak_prominence_factor
    else:
        peak_prominence = peak_prominence_factor *(np.max(data_corrected_averaged_arr[n_data_run])-np.min(data_corrected_averaged_arr[n_data_run])) # Del med 5 virker godt
    peak_width = None #



    peak_idx, peak_props = find_peaks(data_corrected_averaged_arr[n_data_run], height=peak_height, threshold=peak_threshold, distance=peak_distance, prominence=peak_prominence, width=peak_width)
...
def plot_peak_traces(n_start_graph=0, n_graphs = 5, save_graphs=False):
    ...
    for i in range(n_start_graph,n_start_graph+n_graphs):
        ...
        ax[i-n_start_graph].plot(x_av, data_corrected_averaged_arr[i], 'black')

        ax[i-n_start_graph].plot(x_av[peak_idx_arr[i]],data_corrected_averaged_arr[i][peak_idx_arr[i]], marker= 'X', ls='None', color='orange', mec = 'black', mew=0.5, ms=7)
        ...
        ax[i-n_start_graph].annotate(f'Data run number {i}', xy=(0.83, 0.85), xycoords='axes fraction')
    ...
    fig.suptitle(f'Traces and identified peaks for {data_files[data_idx][3:-21]}, runs {n_start_graph}-{n_start_graph+n_graphs}\nProminence: {peak_prominence_factor:.2f}, distance: {peak_distance}', fontsize=16)

    fig.tight_layout(rect=[0.01, 0.01, 1, 0.90])

    if save_graphs:
        dir_name = r'GRAPHPATH\\'+f'{data_files[data_idx][3:-21]}_individual_peak_graphs_averaged'
        ...
        fig.savefig(f'Peak_ident_{data_files[data_idx][3:-21]}_runs_'+f'{n_start_graph}'.zfill(3)+f'_'+f'{n_start_graph+n_graphs}'.zfill(3), dpi=150)
