...
T_ex_string = str(int(np.round(T_factor*100))).zfill(3)
...
dname = r'DATADIR' + T_string + '_varA_varx0'

dir_exists = os.path.exists(dname)
if not dir_exists:
    os.makedirs(dname)
...
print(f'og T_factor er {T_factor}')


# %% Make list of files so can check for duplicates:

file_list = [f for f in os.listdir(dname) if os.path.isfile(os.path.join(dname, f))]

# file_list_set = set(file_list) # Converting to set may be necessary

# %%
A_start = 0.16 # 0.18
A_end = 0.21 # 0.25

n_A = np.int32((np.int32(1000*A_end) - np.int32(1000*A_start)))+1

print(f'number of samples is {n_A}')
...
print(f'Length of m_m0 is {len(m_m0_list)}')

# %%
trim_perc = 0.95 # Where to cut data at
...
# %%
# 
for Amp in tqdm(A_list):

    A_string = f'{Amp*1000:.0f}'.zfill(4)

    for m_m0 in m_m0_list:
        m_m0_string = f'{m_m0*1000:.0f}'.zfill(4)

        if f'T_' + T_string + '_A_'+A_string+f'_'+m_m0_string+'.npz' in file_list:
            """ IF FILE IS ALREADY REGISTERED CONTINUE, MEANS NO OVERWRITING"""
            continue
        else:#
            x0 = np.array([p0_start, m_m0, m0_start])
            x, t = RK4(x0, t0, tf, dt, Amp, omega_kappa, 1, 1)
            np.savez(f'T_' + T_string + '_A_'+A_string+f'_'+m_m0_string+'.npz', x=x[:,trim_index:-1:10], Amp = Amp, x0=x0)

# np.savez('test_x.npz', x=x_natural)
...
# data = np.load(f'x_A_'+A_string+f'_.npz')
